...
public class PlayState extends State{
    ...
    protected static final double LEG_LENGTH = 10000; //distance in meters
    protected int distanceTravelled; //distance travelled by the boats
    //TEAM19-END
    ...
    public void handleInputLogic() {
        ...
        if (player.getTotalLegTime() == 0) {
            if (up_pressed) {
                ...
                distanceTravelled += player.speed; //TEAM19: assuming speed is in m, it adds it to distance travelled
    ...
    public void update(float dt) {
        if ((System.currentTimeMillis() - countDown)/1000 > 3) {
            ...
            checkBoatHealth();
            if ((distanceTravelled > LEG_LENGTH && finishLinePosition == 0)) { //TEAM 19: changed it to distance
                isLegOver();
            }
            finishLeg();
        }
