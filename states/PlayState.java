...
public class PlayState extends State{
    ...
    //TEAM19-START : moved to constant leg distance, and reduced leg length to 5km
    ...
    protected static final int LEG_DISTANCE = 10000;//leg distance in meters
    protected int distanceTravelled;//distance the players boat has travelled
    //TEAM19-END
        ...
        if (player.getTotalLegTime() == 0) {
            if (up_pressed) {
                if (player.getFatigue() > 0 ){
                    ...
                    //TEAM19_START: updates the distance the player has travelled while boosting
                    distanceTravelled += player.acceleration + player.speed;
                }
            } else {
                //TEAM19-START: updates the distance the player has travelled while not boosting
    ...
    public void update(float dt) {
        if ((System.currentTimeMillis() - countDown)/1000 > 3) {
            ...
            checkBoatHealth();
            //TEAM19-START: changed to show leg is over when distance was travelled
            //not when a certain time has passed
            if (distanceTravelled > LEG_DISTANCE && finishLinePosition == 0) {
                isLegOver();
            }
            finishLeg();
        }
